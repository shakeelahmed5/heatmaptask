...
function onClick(regionCode) {
  ...
  if (regionData?.CountryName) {
    messageParts.push(`Country: *${regionData.CountryName.toUpperCase()}*`)
  }

  if (regionData?.Region) {
    messageParts.push(`Region: *${regionData.Region.toUpperCase()}*`)
  }

  messageParts.push(`Sentiment: ${sentimentLabels[sentiment] || 'Unknown'}`)
